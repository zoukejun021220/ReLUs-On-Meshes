...
class VoronoiTrainer:
    ...
    def _beta(self, step: int) -> float:
        cfg = self.config
        stage_a = cfg.stage_a_steps
        stage_b = cfg.stage_b_steps

        if stage_a > 0 and step <= stage_a:
            t = step / max(1, stage_a)
            return cfg.beta_start + t * (cfg.beta_mid - cfg.beta_start)

        if step <= stage_b:
            denom = max(1, stage_b - stage_a)
            t = (step - stage_a) / denom
            return cfg.beta_mid + t * (cfg.beta_end - cfg.beta_mid)

        denom = max(1, cfg.max_steps - stage_b)
        t = (step - stage_b) / denom
        t = min(max(t, 0.0), 1.0)
        return cfg.beta_end + t * (cfg.beta_refine - cfg.beta_end)

    def _stage_loss_weights(self, step: int) -> LossWeights:
        cfg = self.config
        if step <= cfg.stage_a_steps:
            return LossWeights(
                seed=cfg.seed_weight,
                eikonal=cfg.stage_a_eik_weight if cfg.stage_a_eik_weight is not None else cfg.eik_weight,
                lipschitz=cfg.stage_a_lip_weight if cfg.stage_a_lip_weight is not None else cfg.lip_weight,
                interface=0.0,
            )
        if step <= cfg.stage_b_steps:
            return LossWeights(
                seed=cfg.seed_weight,
                eikonal=cfg.stage_b_eik_weight if cfg.stage_b_eik_weight is not None else cfg.eik_weight,
                lipschitz=cfg.stage_b_lip_weight if cfg.stage_b_lip_weight is not None else cfg.lip_weight,
                interface=0.0,
            )
        interface_weight = (
            cfg.stage_c_interface_weight
            if cfg.stage_c_interface_weight is not None
            else cfg.hj_weight
        )
        return LossWeights(
            seed=cfg.seed_weight,
            eikonal=cfg.stage_c_eik_weight if cfg.stage_c_eik_weight is not None else cfg.eik_weight,
            lipschitz=cfg.stage_c_lip_weight if cfg.stage_c_lip_weight is not None else cfg.lip_weight,
            interface=interface_weight,
        )

    def _stage_tv_weight(self, step: int) -> float:
        if step <= self.config.stage_a_steps:
            return self.config.stage_a_tv_weight
        if step <= self.config.stage_b_steps:
            return self.config.stage_b_tv_weight
    ...
    def _grad_align_beta(self, step: int) -> float:
        ...
        t = min(max(t, 0.0), 1.0)
        start = float(self.config.grad_align_beta_start)
        end = float(self.config.grad_align_beta_end)
    ...
    def train(self) -> List[LossTerms]:
        ...
        for step in range(1, self.config.max_steps + 1):
            ...
            seed_loss = seed_anchor_loss(
                self.field_param,
                self.seed_rings_torch,
                margin=self.precomp.seed_radius * self.config.seed_margin_factor,
    ...
    def _update_learning_rate(self, stage: str) -> None:
        ...
        target = {
            ...
            "main": self.config.lr_main,
            "refine": self.config.lr_refine,
        }[stage]
