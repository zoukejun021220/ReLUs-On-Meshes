...
This version patches the optimization to limit iterations properly.
"""
import numpy as np
import torch
import time
...
def test_2_iterations(mesh_path):
    ...
    try:
        ...
        # Get loss history from the direct training
        if 'direct' in history:
            losses = history['direct']['total_loss']
            print(f"\nLoss values from {len(losses)} iterations:")
            for i, loss in enumerate(losses):
                print(f"  Iteration {i+1}: {loss:.4f}")
