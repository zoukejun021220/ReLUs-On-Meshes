#!/usr/bin/env python3
"""
Optimized unified training script with fully vectorized operations.
Ensures CUDA usage and eliminates Python loops for maximum performance.
"""
...
from loss_groundup_optimized import MeshLossOptimized, set_pinned_values_vectorized
...
def train_relu_mesh_optimized(mesh_path, config):
    """Optimized training function with full CUDA support."""
    ...
    # Convert to torch tensors directly on device
    verts = torch.from_numpy(vertices).float().to(device)
    faces_tensor = torch.from_numpy(faces).long().to(device)
    
    # Initialize optimized loss module
    print("Initializing optimized loss module...")
    mesh_loss = MeshLossOptimized(verts, faces_tensor, device)
    ...
    # Initialize field directly on device
    N = len(vertices)
    f_values = torch.randn(N, 6, device=device) * 0.1
    
    # Vectorized pinned value setting
    set_pinned_values_vectorized(f_values, pinned_indices)
    ...
    # Training loop
    print(f"\nStarting optimized training for {config['n_iters']} iterations...")
    print("="*70)
    
    start_time = time.time()
    
    # Pre-allocate pinned indices tensor for reuse
    pinned_tensor = torch.tensor(pinned_indices[:6], device=device)
    channel_indices = torch.arange(len(pinned_tensor), device=device)
    ...
    for it in range(config['n_iters']):
        ...
        if it % config['print_every'] == 0:
            ...
            if device.type == 'cuda':
                ...
                mem_info = f" | GPU: {allocated:.1f}/{reserved:.1f}GB"
        ...
        # Re-pin vertices (vectorized)
        with torch.no_grad():
            f_values[pinned_tensor] = 0.0
            f_values[pinned_tensor, channel_indices] = 1.0
...
def main():
    parser = argparse.ArgumentParser(
        description="Optimized unified training script for ReLU mesh optimization",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Basic usage (auto-detects CUDA)
  python train_unified_optimized.py mesh.vtk
  
  # Force CPU usage
  python train_unified_optimized.py mesh.npz --device cpu
  
  # High-performance training
  python train_unified_optimized.py kitty.vtk --iters 100000 --mesh-type kitty
        """
    )
    ...
    if args.output is None:
        ...
        args.output = f"{base}_results_optimized.npz"
    ...
    # Run training
    try:
        results = train_relu_mesh_optimized(args.mesh, config)
